...
const moveFocus = (direction, event) => {
  if (document.activeElement instanceof Element) {
    ...
    if (stash) {
      const next =
          stash[{up: 'previousElementSibling',
                 down: 'nextElementSibling'}[direction]];
      if (next) {
        next.focus();
  ...
  const first = document.getElementById(
      'stash-list')[{up: 'lastElementChild',
                     down: 'firstElementChild'}[direction]];
...
const init = async function() {
  ...
  const vm = new Vue({
    ...
    data: {
      ...
      topUpTabsLabel: mode == 'selection' ?
          highlightedTabs.length.toString() + ' tabs' :
          'current tab'
    },
    methods: {
      stashWindow: function() {
        saveStash(
            this.stashNameWindow, windowTabs,
      ...
      stashTabs: function() {
        saveStash(
            this.stathNameTabs, highlightedTabs,
  ...
  chrome.storage.onChanged.addListener(async(changes, areaName) => {
    const [stashes, messages] =
        await Promise.all([getStashes(), getMessages()]);
    vm.stashes = stashes;
